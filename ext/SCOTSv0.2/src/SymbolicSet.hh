...
public:
  ...
  inline bool i_to_bdd(const std::vector<double>& astate, BDD & bdd){
    for(int k=0; k < m_dim; ++k) {
        const abs_type abs_id = std::round(std::abs(astate[k]));
  ...
  inline BDD i_to_bdd(const std::vector<double>& astate) const {
    BDD bdd = m_bdd_interval[0].int_to_bdd(std::round(std::abs(astate[0])));
    for(int k=1; k < m_dim; ++k) {
      bdd = bdd & m_bdd_interval[k].int_to_bdd(std::round(std::abs(astate[k])));
    }
    return bdd;
  }
  ...
  // find distinct/unique subset on inputs based on bdd restriction 
  template<class state_dim>
  void get_distinct_inputs(const Cudd& manager,
                                  const BDD& bdd, 
                                  const state_dim x_dim,
    ...
    if(domain.size()==0) {
      domain.resize(x_dim);
      std::iota(std::begin(domain),std::end(domain),0);
      codomain.resize(m_dim-x_dim);
      std::iota(std::begin(codomain),std::end(codomain),x_dim);
    } else {
      for(int i=0; i<m_dim; i++) {
        if(std::find(std::begin(domain), std::end(domain), i) == std::end(domain))
          codomain.push_back(i);
    ...
    // std::cout <<"set domain size "<< set_dom.size() << "\n";
    // std::cout <<"set codomain size" << set_dom.size() << "\n";
    // to store the bdd representation of input
    std::vector<BDD> distinct_inputs;
    
    /* extract bdd with restriction */
    // loop all over the index of all of states
    for(abs_type id =0; id < set_dom.size(); id++){
      // convert to BDD
      BDD restricted = bdd & set_dom.id_to_bdd(id);     
      // extract the restricted
      restricted = restricted.ExistAbstract(set_dom.get_cube(manager));  
      // store it on the bdd's vector if it is unique
      if(!distinct_inputs.size()){
        // std::cout << "first initialization ";
        distinct_inputs.push_back(restricted);
      }

      bool found = false;
      for(unsigned i=0; i<distinct_inputs.size(); i++)
      {
        if(debug_mode){
          std::vector<double> result{};
          set_codom.bdd_to_grid_points(manager, restricted, result);
          ...
          set_codom.bdd_to_grid_points(manager, distinct_inputs[i], result);
        ...
        if(restricted == distinct_inputs[i]){
          found = true;
          break;
        }
      }
    ...
    // convert to double to get readable representation
    for(abs_type in = 0; in < distinct_inputs.size(); in++){
      std::vector<double> result{};
      // convert
      set_codom.bdd_to_grid_points(manager, distinct_inputs[in], result);
      // push
      results.push_back(result);
    }
  }

  // additional method to obtain distict inputs
  template<class state_dim>
  std::vector<std::vector<double>> get_distinct_inputs(const Cudd& manager,
                                  const BDD& bdd, 
                                  const state_dim x_dim,  
                                  std::vector<int> domain = {}) const {
    std::vector<std::vector<double>> results;
    get_distinct_inputs(manager, bdd, x_dim, results, domain);
    return results;
  }
