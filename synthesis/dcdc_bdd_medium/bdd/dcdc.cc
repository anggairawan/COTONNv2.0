...
int main() {
  ...
  scots::SymbolicSet ss_input(manager,input_dim,input_type{{1}},input_type{{2}},input_type{{1}});
  ss_input.print_info();
  ...
  auto avoid = [&ss_pre](const abs_type& idx) {
    for(size_t i=0; i<H.size(); i++) {
      if(idx == H[i])
      {
        std::cout << "Avoid state: " << idx << "\n";
        return true;
      }
    }
    return false;
  ...
  tt.tic();
  size_t no_trans;
  BDD TF = sym_model.compute_gb(manager,system_post,radius_post,avoid,no_trans);
  tt.toc();
