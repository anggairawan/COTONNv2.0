...
public:
  ...
  std::vector<state_type> get_domain() const {
    ...
    std::vector<state_type> domain(abs_domain.size());
    for(abs_type i=0; i<abs_domain.size(); i++) {
      state_type x;
      m_state_grid.itox<state_type>(abs_domain[i],x);
      domain[i]=x;
    }
    return domain;
  }
  ...
  // 
  UniformGrid get_ss_uniformgrid() const {
    return m_state_grid;
  }

  // 
  UniformGrid get_is_uniformgrid() const {
    return m_input_grid;
  }
