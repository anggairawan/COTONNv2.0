...
import sys
reload(sys.modules['StaticController'])
...
class Importer:
      ...
      def readStaticController(self, filename):
            ...
            for i in range(len(matrix_data)):
                ...
                state_dim = int(ctl.getStateSpaceDim());
                if(self.det):
                    s = int(split_data[0].replace(' ',''))
                    u = int(split_data[1].replace(' ',''))
                #elif(self.det ==  NNencode.MAKE_DET):
                else:
                    s = int(split_data[0].replace(' ',''))
                    # take all the possible valid inputs
                    if '' in split_data:
                        split_data.remove('')
                    u = [int(x) for x in split_data[1:]]
                    """
                    if(self.dof):
                        s = [int(x) for x in split_data[0:state_dim]]
                        u = [int(x) for x in split_data[state_dim:-1]]
                    else:
                        s = int(split_data[0].replace(' ',''))
                        # u = int(split_data[1].replace(' ',''))
                        # take all the possible valid inputs
                        u = [int(x) for x in split_data[1:-1]]
                    """
            ...
            ctl.setSize()
            ctl.setStateNoGridPoints();
            ctl.setInputNoGridPoints();

            ctl.setDeterministic(self.det);
