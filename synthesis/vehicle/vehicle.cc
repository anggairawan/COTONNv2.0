...
int main() {
  ...
  auto avoid = [&O,ss,s_eta](const abs_type& idx) {
    bool flag = false;
    state_type x;
    ss.itox(idx,x);
    ...
    for(size_t i=0; i<15; i++) {
      if ((O[i][0]-c1) <= x[0] && x[0] <= (O[i][1]+c1) && 
          (O[i][2]-c2) <= x[1] && x[1] <= (O[i][3]+c2))
        flag = true;
    }
  ...
  std::ofstream myfile;
  std::ofstream mylog;
  myfile.open ("../../simulation/matlab/winning_x_vehicle.m");
